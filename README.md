...
- Locks on idle, on minimise, on losing focus, or on being hidden to the tray — each configurable, and stored inside the encrypted vault so that editing a config file cannot switch them off.
