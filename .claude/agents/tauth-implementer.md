...
- **An assertion of absence must be falsifiable.** A test asserting nothing happened has to observe something a live implementation would change — a call count, a recorded request, a state flag. Appending to a list from inside the very coroutine you canceled proves nothing about what the code does after the cancel.
